function three(data, option = {}) {
  ...
  three.prototype.getRootMenu = function () {
    for (var m of this.dataSource) {
      m.selected = this.selected && this.selected.indexOf(m.promptId) >= 0 ? true : false;
      m.checked = this.children && this.children.indexOf(m.promptId) >= 0 ? true : false;
      m.expand = true;
      if (m.parentId=="0") {
  ...
  three.prototype.pushNode = function (id, node) {
    for (var m of this.dataSource) {
      if (m.parentId == id) {
        ...
        this.pushNode(node.children[node.children.length - 1].promptId, node.children[node.children.length - 1]);
      }
    }
  }
  ...
  three.prototype.sort = function (list) {
    list = list.sort(function (a, b) {
      return a.promptName.localeCompare(b.promptName,"zh");
    })
